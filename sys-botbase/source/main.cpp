...
extern "C"
{
    ...
    void __appInit(void) {
        ...
        if (R_FAILED(pmdmntInitialize())) fatalThrow((Module_Libnx));
        if (R_FAILED(ldrDmntInitialize())) fatalThrow((Module_Libnx));
        if (R_FAILED(pminfoInitialize())) fatalThrow((Module_Libnx));
        if (R_FAILED(socketInitializeDefault())) fatalThrow((Module_Libnx));
        rc = capsscInitialize();
        if (R_FAILED(rc)) fatalThrow(rc);
        if (R_FAILED(viInitialize(ViServiceType_Default))) fatalThrow((Module_Libnx));
    ...
    int argmain(int argc, char** argv)
    {
        ...
        if (!strcmp(argv[0], "clickSeq"))
        {
            ...
            char* seqNew = (char*)malloc(sizeArg);
        ...
        if (!strcmp(argv[0], "configure")) {
            ...
            if (!strcmp(argv[1], "controllerType")) {
                ...
                controllerInitializedType = (HidDeviceType)fControllerType;
            }
        }
